...
LOGQ = q("edit_register.xlsx About", "a new line is added whenever the call on an edit is revisited; from then on the edit's latest line is its call, with that line's line change and state.")
...
V = [
 ...
 table("register_trap_ps04", ["PS-04"], "; ".join([ROWQ, PAGEQ, LOGQ, DEFQ]),
   "csv.read_rows + table_equals on the PS-04 row only (page_number as a number, verdict as text; a duplicated PS-04 row fails). PS-04's only edit, ED-03 (-22, as long as the passage), was logged ACCEPTED on 2026-08-04 (first read, and so in the Tally sheet) and DEFERRED on 2026-09-08; it stands as its latest line, so PS-04 keeps its 22 lines: page 2, EDIT_DEFERRED. A run that applies every ACCEPTED line, reads an edit's first line, or folds the second read onto the Tally as additions floors it instead. Isolated so that reading is named by its own check."),
 table("register_trap_ps07", ["PS-07"], "; ".join([ROWQ, PAGEQ, EVERYQ, FLOORQ, PRIOQ]),
   "csv.read_rows + table_equals on the PS-07 row only. PS-07 carries two edits, ED-05 (-21, 2026-08-05) and ED-09 (-5, 2026-09-09), both standing ACCEPTED; together they take its 25 drafted lines to -1, so it is held at 1 line: page 3, LENGTH_FLOORED. A run that keeps one line per passage gets 20 or 4 lines and a geometric verdict. Isolated so that reading is named by its own check."),
 table("register_trap_ps10", ["PS-10"], "; ".join([ROWQ, PAGEQ, LOGQ, EVERYQ]),
   "csv.read_rows + table_equals on the PS-10 row only. PS-10's only edit, ED-07 (+14), was logged DEFERRED on 2026-08-06 (so the Tally marks PS-10 deferred) and ACCEPTED on 2026-09-09; it stands ACCEPTED, so PS-10 runs 22 lines (114-135) across the page 4/5 break: page 4, STRADDLES_BREAK. A run that keeps the Tally's deferred flag, treats any DEFERRED line as a deferred edit, or reads the edit's first line, records EDIT_DEFERRED. Isolated so that reading is named by its own check."),
 table("register_rows", others, "; ".join([ROWQ, PAGEQ, LAYQ, LOGQ, PUSHQ, DEFQ]),
   "csv.read_rows + table_equals on the other 27 passages cell by cell, with the row set locked to the ledger's 30 passage ids (a missing, extra or duplicated passage fails) and the column set closed to the three header columns. Row order is not graded.", row_set=True),
