...
positive-only):
  ...
  test_deliverable (positive)     every graded assertion passes on the workspace.
                                  Each outcome is recorded to checks.jsonl, which
                                  score.py turns into the reward.
...
The negative lanes mutate COPIES; the workspace itself is never modified. Every check in
this task is deterministic, so the lanes replay the whole spec. One pytest per assertion/case, so
Harbor's per-test grid (and the CTRF report) names exactly what failed. The spec in
`verifier.json` and the engine in `rl_world_verifiers/` are copies of what the task
harness runs, so a result here means the same thing it means there.
"""
...
from rl_world_verifiers.verifiers import (  # noqa: E402
...
DETERMINISTIC = [d for d in SPEC.verifiers if d.assertion.type == "deterministic"]
...
def _record(definition, outcome, infra_error=False):
    """Appends one check's outcome for score.py.

    `infra_error` marks a check that never produced a verdict because the engine
    raised VerifierInfrastructureError; score.py refuses to grade a run containing
    one. Every check in this task is deterministic, so it is not expected here.
    """
    try:
        ...
        with CHECKS_PATH.open("a", encoding="utf-8") as handle:
            handle.write(json.dumps({
                ...
                "detail": str(outcome.get("error") or outcome.get("reason") or "")[:300],
            }) + "\n")
    except Exception:  # bookkeeping must never fail a graded run
        pass
...
#: deterministic check names per graded file
CHECKS_ON = {
    path: sorted(raw["name"] for raw in _RAW
                 if _src(raw).get("arguments", {}).get("path") == path
                 and (raw.get("assertion") or {}).get("type") == "deterministic")
...
#: lane reads a fully correct gold as "figure is decorative".
TOL_BUMP = {}
for raw in _RAW:
...
#: the key-set guard(s) on results.json
...
def _failed(root):
    """Names of the deterministic checks that fail when replayed against `root`."""
    registry = SourceRegistry(Path(root))
    failed = set()
    for definition in DETERMINISTIC:
...
def test_deliverable(definition):
    try:
        ...
        _record(definition, {"success": False, "error": str(exc)}, infra_error=True)
        raise AssertionError(
            f"{definition.name}: INFRASTRUCTURE FAILURE (not graded) — {exc}"
        ) from exc
    _record(definition, outcome)
