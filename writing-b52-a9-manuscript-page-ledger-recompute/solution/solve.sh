...
if isinstance(golden, dict) and isinstance(golden.get("steps"), list):
    # golden_trajectory.json is a promoted reward-1.0 GLM-5.2 run in ATIF form.
    # Replay its steps as the oracle record, labelled as a replay so it is never
    # mistaken for a live agent run.
    traj = {
        "schema_version": golden.get("schema_version", "ATIF-v1.5"),
    ...
    print(f"oracle: wrote {log/'trajectory.json'} ({len(traj['steps'])} replayed steps)")
    raise SystemExit(0)
