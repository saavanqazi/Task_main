...
| 6 | v7 | the reference's trap without signposts: ledger `passage_id, draft_line, drafted_lines` in flagging order; PS-16 a bare repeated id at file lines 17 and 39; definition = one parenthetical in the spec + one in the submission format; leads "none has been merged or dropped" and "Tomas got the flags right; it's the pages that are stale"; new `Draft pages` sheet (41 rows, PS-16 twice, all values true of the draft). Same 58 decisions, same gold, same 13 checks | pending (`b52a9-glm5x-run7`) | |

Gold v7 (= v6's layout; only the ledger's encoding and the helper changed): 12 straddling / 7 deferred / 1 floored /
20 wholly / final page 18 (539 lines). Local: derive_gold clean (draft contiguity, stretch adjacency, Draft pages and
Not in all asserted), discrimination 50/50 (18 readings distinct), oracle 1.0, nop 0.0, symlinks 0.0, planted hooks 0.0,
extra key 0.923077. The fixture is generated by `tools/build_fixture.py`; change data there, never by hand.

**If v7 is still 5/5:** the reference's mechanism, reproduced faithfully, does not move this model on this task
family, and further rounds are not the answer. Report that in README §8 with the six batteries as the evidence
(30 of 30 passes across v2-v7, every trajectory read), classify the task as out of band for GLM-5.2, and let the
gate decide. Do not add a seventh trap.
