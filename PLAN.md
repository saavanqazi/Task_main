...
| 4 | v5 | no state column: 32 decisions in the readers' own words (15 wordings: revised counts in words, "Back to Ines's call.", parks and turn-downs of earlier acceptances); three calls on the About sheet (accepted / deferred / withdrawn); Tally replaced by Tomas's `Not in` sheet (deferred + withdrawn mixed, no state), vouched truthfully; 5 trap checks (PS-04, PS-07, PS-10, PS-20, PS-24), 9 checks | pending (`b52a9-glm5x-run5`) | |

Gold v5: 11 straddling / 4 deferred / 1 floored / 14 wholly / final page 15. Local: derive_gold clean (every Log wording
declared in DECISIONS; Not in sheet asserted equal to the edits not accepted), discrimination 41/41 (13 readings
distinct), oracle 1.0, nop 0.0, symlinks 0.0, planted hooks 0.0, extra key 0.888889.
The fixture is generated by `tools/build_fixture.py`; change data there, never by hand in the workbook.

Hardening levers for round 5 (if v5 is still 4-5/5):

- **H1:** a second turn-down that reads like a park ("Not this time; the author can raise it again." = withdrawn)
  and a park that reads like a turn-down ("Not now; ask the author." = deferred), each defined by the About sheet's
  "stays open" test. Declare both in `DECISIONS`.
- **H2:** a return to an earlier call two decisions back ("Back to the first call." after two changes).
- **H3:** move the three-call definition from the About sheet's second paragraph into a footnote row under the Log.

Easing levers for round 5 (if v5 is 0/5):

- **E1:** drop the `Not in` sheet and its lead.
- **E2:** add a `withdrawn` example to the About sheet ("e.g. 'No, keep it as drafted.'").
- **E3:** add `WITHDRAWN` handling to `layout_spec.md` in words ("a withdrawn edit leaves no mark on its passage").
