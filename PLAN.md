...
| 2 | v3 | register made a dated **log**: an edit stands as its latest line. Second read re-logs ED-01 (+4 → +6), ED-03 (PS-04's exact cut, ACCEPTED → DEFERRED) and ED-07 (PS-10, DEFERRED → ACCEPTED); ED-09 stays as PS-07's second edit. PS-11 17 lines (ends on line 150). Lead: "Nothing has been struck from it; every line in it is a call one of us made." New trap check `register_trap_ps10`; 7 checks | **5/5** (`b52a9-glm5x-run3`; run2 = Windows cp1252 crash, INFRA, discarded) | too easy: all 10 trajectories restate each spec rule and apply it; nothing hides in 12 rows |
| 3 | v4 | reference pattern: register becomes `edit_register.xlsx` (Log / About / Tally); the latest-line rule lives on the About sheet; Ines's first-read Tally is vouched for ("right as far as it goes, and only Tomas's second read needs folding in"); 30 passages, 29 log lines; second read re-confirms ED-06, revises ED-01 and ED-13, retracts exact cuts ED-03 and ED-17, accepts ED-07, adds ED-09 and 4 more | **5/5** (`b52a9-glm5x-run4`) | too easy: every run cross-checked against the Tally, which confirmed the right method |
| 4 | v5 | no state column: 32 decisions in the readers' own words (15 wordings: revised counts in words, "Back to Ines's call.", parks and turn-downs of earlier acceptances); three calls on the About sheet (accepted / deferred / withdrawn); Tally replaced by Tomas's `Not in` sheet (deferred + withdrawn mixed, no state), vouched truthfully; 5 trap checks (PS-04, PS-07, PS-10, PS-20, PS-24), 9 checks | **5/5** (`b52a9-glm5x-run5`, 16 min) | too easy: every run hand-encoded the 32 decisions correctly and re-checked against Not in |
| 5 | v6 | stacked: (1) 58 decisions on 45 edits in 30 wordings incl. "Ditto." ×6, "Same call as ED-xx." ×4, "As Ines had it.", "Take it, at the number Ines had."; no confirming helper; (2) the reference's ledger trap: 41 flags in flagging order with `position`, second-read flags appended, PS-16 in two stretches (24 + 9) with the second near the bottom; 40 passages; 9 trap checks, 13 checks | **5/5** (`b52a9-glm5x-run6`, 20 min) | too easy: the `note` and `position` columns signposted the two-stretch passage ("some are duplicates (stretches of the same passage)" was one run's first observation); the 58 decisions were hand-encoded without a slip |
| 6 | v7 | the reference's trap without signposts: ledger `passage_id, draft_line, drafted_lines` in flagging order; PS-16 a bare repeated id at file lines 17 and 39; definition = one parenthetical in the spec + one in the submission format; leads "none has been merged or dropped" and "Tomas got the flags right; it's the pages that are stale"; new `Draft pages` sheet (41 rows, PS-16 twice, all values true of the draft). Same 58 decisions, same gold, same 13 checks | **5/5** (`b52a9-glm5x-run7`, 16 min) | too easy: the reference's mechanism, reproduced without signposts, did not move this model |
| 7 | v8 | decisions move from a Log sheet into `edit_thread.txt`: 19 messages, each reply quoting the full history (1,670 lines, 361 quoted decision lines, 18 levels of `>`); About says quoted lines are the earlier message; `Not in` dropped; `Edits` sheet carries the proposed changes. Same ledger, gold and 13 checks; W18 (quoted lines counted) added | pending (`b52a9-glm5x-run8`) | |

Gold v8 (= v7's; only the decisions' carrier changed): 12 / 7 / 1 / 20 / final page 18 (539 lines). Local: derive_gold
clean (draft contiguity, stretch adjacency, Draft pages, thread quotes and edit coverage all asserted), discrimination
51/51 (19 readings distinct), oracle 1.0, nop 0.0, symlinks 0.0, planted hooks 0.0, extra key 0.923077.
The fixture is generated by `tools/build_fixture.py`; change data there, never by hand.

**If v8 is still 5/5:** a data-shape trap done as the reference did it (v7) and a change of input kind (v8) both
failed to move this model on this task family, and further rounds are not the answer. Report that in README §8 with the six batteries as the evidence
(35 of 35 passes across v2-v8, every trajectory read), classify the task as out of band for GLM-5.2, and let the
gate decide. Do not add a ninth build.

Hardening levers for round 7 (only if the gate asks for one more round):

- **H1:** more reference decisions: "Ditto." chains (a Ditto. after a Ditto.), "Same call as" targets that
  themselves changed later (the call as it *then* stood is what counts), a "Same call as" on the first read
  pointing at an edit Tomas later reverses.
- **H2:** a second two-stretch passage whose stretches are both appended (neither on the first read).
- **H3:** a decision whose number is relative to what last stood ("Two more lines out."), defined on the About
  sheet as "a number given as more or fewer is taken from the edit's number as it last stood".

Easing levers for round 7 (if v7 is 0/5):

- **E1:** drop the "Ditto." and "Same call as" wordings (replace each with the plain call it resolves to).
- **E2:** put the ledger in manuscript order (keep `position`) and move PS-16's second stretch next to its first.
- **E3:** drop the `Not in` sheet and its lead.

After **any** lever:

```bash
python3 ../tools/build_fixture.py .             # rebuild ledger + edit_register.xlsx from tools/build_fixture.py's tables
python3 tests/derive_gold.py --write            # new gold (in the package dir; needs openpyxl)
