...
| 3 | v4 | reference pattern: register becomes `edit_register.xlsx` (Log / About / Tally); the latest-line rule lives on the About sheet; Ines's first-read Tally is vouched for ("right as far as it goes, and only Tomas's second read needs folding in"); 30 passages, 29 log lines; second read re-confirms ED-06, revises ED-01 and ED-13, retracts exact cuts ED-03 and ED-17, accepts ED-07, adds ED-09 and 4 more | pending (`b52a9-glm5x-run4`) | |

Gold v4: 9 straddling / 5 deferred / 1 floored / 15 wholly / final page 14. Local: derive_gold clean (Tally asserted
equal to the first read), discrimination 39/39, oracle 1.0, nop 0.0, symlinks 0.0, planted hooks 0.0, extra key 0.857143.
The fixture is generated by `tools/build_fixture.py`; change data there, never by hand in the workbook.

Hardening levers for round 4 (if v4 is still 4-5/5), mildest first; each keeps the definitions in the rules:

- **H1:** the Tally also gives each passage's *resulting length* and first-read page, so folding in the second read
  looks like a small patch to a finished register.
...
Easing levers for round 4 (if v4 is 0/5), mildest first:

- **E1:** soften the Tally lead to "Ines's Tally sheet adds up her first read." (drop "only Tomas's second read needs folding in").
- **E2:** drop the ED-01 revision line (fewer re-logs; the two state flips stay).
- **E3:** move the About sheet's log definition back into `layout_spec.md` (the v3 wording; last resort)
